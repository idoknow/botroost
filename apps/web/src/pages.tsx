  ...
  return <Stack className="endpoint-console">
    ...
    {canOperate&&endpoint.providerId==='napcat'?<Tabs defaultValue="overview" className="endpoint-tabs">
      ...
      <TabsContent value="overview" className="endpoint-tab-panel">
        <Card className="qq-account-card"><div className="card-section-heading"><div><h2>QQ account</h2><p className="muted">The QQ session managed by this NapCat endpoint.</p></div>{loggedIn&&<Badge good>Logged in</Badge>}</div>{loggedIn?<div className="info-grid"><Info label="Nickname">{String(qq?.nickname??qq?.nick??onebot?.loginInfo?.nickname??'QQ account')}</Info><Info label="QQ number">{String(qq?.uin??qq?.uid??onebot?.loginInfo?.user_id??'—')}</Info><Info label="Friends">{onebot?.friendCount??onebot?.friends?.length??'—'}</Info><Info label="Groups">{onebot?.groupCount??onebot?.groups?.length??'—'}</Info></div>:<div className="qq-login-panel"><div className="endpoint-qr">{qr.data?.qrcode?<QRCodeSVG value={qr.data.qrcode} size={184} role="img" aria-label="NapCat QR code"/>:<span>Waiting for a QR code.</span>}</div><div className="qq-login-copy"><h3>Scan with the QQ mobile app</h3><p className="muted">Open QQ, scan this code, then confirm the login on your phone. The status updates automatically; refresh only if the code expires.</p><Button busy={busy==='qr'} disabled={Boolean(endpoint.activeOperationId)} onClick={()=>run('qr',async()=>{await wait(await api.mutate<Operation>(`/endpoints/${id}/napcat/login-qrcode`),'QR refresh failed');await Promise.all([qr.refresh(),napcat.refresh(),q.refresh()])})}>Refresh QR code</Button></div></div>}<div className="protocol-runtime"><div><h3>Protocol runtime</h3><p>NapCat provides the OneBot 11 interface for this QQ account. It is not a separate service; configure how applications connect under Connections.</p></div><div className="runtime-facts"><Info label="Runtime">NapCat</Info><Info label="Runtime version">{String(onebot?.version?.app_version??'Awaiting runtime report')}</Info></div></div></Card>
        {loggedIn&&<Card><Tabs value={directory} onValueChange={value=>setDirectory(value as 'friends'|'groups')}><TabsList className="product-tabs-list"><TabsTrigger className="product-tabs-trigger after:hidden" value="friends">Friends ({onebot?.friendCount??onebot?.friends?.length??0})</TabsTrigger><TabsTrigger className="product-tabs-trigger after:hidden" value="groups">Groups ({onebot?.groupCount??onebot?.groups?.length??0})</TabsTrigger></TabsList><TabsContent value="friends"><Table headers={['Nickname','Remark','QQ']} rows={(onebot?.friends??[]).map(friend=>[String(friend.nickname??'—'),String(friend.remark??'—'),String(friend.user_id)])}/></TabsContent><TabsContent value="groups"><Table headers={['Group','Members','ID']} rows={(onebot?.groups??[]).map(group=>[String(group.group_name??'—'),String(group.member_count??'—'),String(group.group_id)])}/></TabsContent></Tabs></Card>}
      </TabsContent>
