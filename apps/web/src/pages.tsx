...
function TrafficPanel({traffic}:{traffic?:ProtocolTraffic}){
  ...
  return <>
    <Card className="traffic-card"><div className="card-section-heading"><div><h2>Real-time message traffic</h2><p className="muted">Five-second aggregate samples from the managed NapCat runtime.</p></div><Badge good={traffic.status==='ok'}>{traffic.status==='ok'?'Live':traffic.status==='partial'?'Partial':'Unavailable'}</Badge></div>
