...
type NapCatStatus={qq:null|Record<string,unknown>;onebot:null|{loginInfo?:Record<string,unknown>;friends?:Record<string,unknown>[];friendCount?:number;groups?:Record<string,unknown>[];groupCount?:number;version?:Record<string,unknown>;config?:{websocketClients?:WsClient[];websocketServers?:WsServer[]}}};
export function EndpointDetail({session,id}:{session:Session;id:string}){
  ...
  const loggedIn=Boolean(qq?.uin||qq?.uid||onebot?.loginInfo?.user_id);
  ...
  const settings=<EndpointSettings endpoint={endpoint} session={session} refresh={q.refresh}/>;

  return <Stack className="endpoint-console">
    <PageHeading kicker="OneBot protocol endpoint · NapCat driver" title={endpoint.name} description={`Hosted by ${endpoint.node?.name??'an unassigned agent node'} · ${endpoint.id}`}/>
    ...
    {canOperate&&endpoint.providerId==='napcat'?<Tabs defaultValue="overview" className="endpoint-tabs">
      <TabsList variant="line" className="endpoint-tabs-list">
        <TabsTrigger value="overview">Overview</TabsTrigger>
        <TabsTrigger value="connections" disabled={!loggedIn}>Connections</TabsTrigger>
        <TabsTrigger value="logs">Logs</TabsTrigger>
        <TabsTrigger value="settings">Settings</TabsTrigger>
      </TabsList>
      <TabsContent value="overview" className="endpoint-tab-panel">
        <div className="two-col">
          <Card><h2>QQ account</h2>{loggedIn?<div className="info-grid"><Info label="Nickname">{String(qq?.nickname??qq?.nick??onebot?.loginInfo?.nickname??'QQ account')}</Info><Info label="QQ number">{String(qq?.uin??qq?.uid??onebot?.loginInfo?.user_id??'—')}</Info><Info label="Friends">{onebot?.friendCount??onebot?.friends?.length??'—'}</Info><Info label="Groups">{onebot?.groupCount??onebot?.groups?.length??'—'}</Info></div>:<div className="qr-row">{qr.data?.qrcode?<QRCodeSVG value={qr.data.qrcode} size={184} role="img" aria-label="NapCat QR code"/>:<span>Waiting for a QR code.</span>}<Button busy={busy==='qr'} disabled={Boolean(endpoint.activeOperationId)} onClick={()=>run('qr',async()=>{await wait(await api.mutate<Operation>(`/endpoints/${id}/napcat/login-qrcode`),'QR refresh failed');await Promise.all([qr.refresh(),napcat.refresh(),q.refresh()])})}>Refresh QR code</Button></div>}</Card>
          <Card><h2>NapCat OneBot implementation</h2><Info label="Implementation">{String(onebot?.version?.app_name??'Not reported')}</Info><Info label="Implementation version">{String(onebot?.version?.app_version??'Not reported')}</Info></Card>
        </div>
        {loggedIn&&<Card><Tabs value={directory} onValueChange={value=>setDirectory(value as 'friends'|'groups')}><TabsList variant="line"><TabsTrigger value="friends">Friends ({onebot?.friendCount??onebot?.friends?.length??0})</TabsTrigger><TabsTrigger value="groups">Groups ({onebot?.groupCount??onebot?.groups?.length??0})</TabsTrigger></TabsList><TabsContent value="friends"><Table headers={['Nickname','Remark','QQ']} rows={(onebot?.friends??[]).map(friend=>[String(friend.nickname??'—'),String(friend.remark??'—'),String(friend.user_id)])}/></TabsContent><TabsContent value="groups"><Table headers={['Group','Members','ID']} rows={(onebot?.groups??[]).map(group=>[String(group.group_name??'—'),String(group.member_count??'—'),String(group.group_id)])}/></TabsContent></Tabs></Card>}
      </TabsContent>
