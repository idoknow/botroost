...
export function resourceState(sample:ResourceUsage|undefined,node:string,failed:boolean,now:number):'live'|'stale'|'missing'|'stopped'|'unavailable'{
 if(!sample)return 'missing';
 if(sample.status==='unavailable')return 'unavailable';
