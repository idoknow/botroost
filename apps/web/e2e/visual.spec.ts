...
for(const colorScheme of ['light','dark'] as const){
  for(const viewport of viewports){
    test(`visual fixture ${colorScheme} ${viewport.name}`,async({page})=>{
      ...
      await expect(page.getByText('Protocol runtime')).toBeVisible();await expect(page.getByText('NapCat OneBot implementation')).toHaveCount(0);await expect(page.getByText('API available')).toHaveCount(0);await expect(page.getByText('OneBot connecting')).toHaveCount(0);
