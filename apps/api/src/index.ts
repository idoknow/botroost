...
export function buildApi(options: ApiOptions = {}): FastifyInstance {
  ...
  api.setErrorHandler((error, _request, reply) => {
    ...
    if (error instanceof DatabaseError) return reply.code(error.code === "not_found" ? 404 : 409).send({ error: { code: error.code, message: error.message } });
  ...
  api.addHook("preHandler", async request => {
    if (["GET", "HEAD", "OPTIONS"].includes(request.method) || request.url === "/api/v1/auth/login") return;
  ...
  api.post("/api/v1/nodes/enrollment-tokens", async (request, reply) => { const current = await authorize(request, "manage-nodes"); const raw = randomBytes(32).toString("base64url"); const expiresAt = new Date(Date.now() + 15 * 60_000); return reply.code(201).send({ ...(await db.createEnrollment(current.workspaceId, digest(raw), expiresAt)), token: raw }); });
