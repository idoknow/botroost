...
export function buildApi(options: ApiOptions = {}): FastifyInstance {
  ...
  api.get("/api/v1/endpoints/:id/napcat/login-qrcode", async (request, reply) => { const current = await principal(request); const metadata = await db.endpointNapcatMetadata(current.workspaceId, idParams.parse(request.params).id); const login = metadata.login as Record<string, unknown>|undefined; const qrcode = typeof login?.qrcode === "string" ? login.qrcode : null; return qrcode ? { qrcode } : reply.code(404).send({ error: { code: "not_found", message: "QR login is not available" } }); });
  api.get("/api/v1/endpoints/:id/napcat/status", async request => { const current = await principal(request); const metadata = await db.endpointNapcatMetadata(current.workspaceId, idParams.parse(request.params).id); return { qq: metadata.qq ?? null, login: metadata.login ?? null, onebot: metadata.onebot ?? null }; });
