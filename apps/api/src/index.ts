...
export function buildApi(options: ApiOptions = {}): FastifyInstance {
  ...
  api.get("/api/v1/workspaces/current/settings/resend",async request=>db.workspaceResendSettings((await authorizeContract(request,"settings:read")).workspaceId));
  api.put("/api/v1/workspaces/current/settings/resend",async request=>{const current=await authorizeContract(request,"settings:manage"),body=resendSettingsInput.parse(request.body),input={enabled:body.enabled,recipient:body.recipient,from:body.from,graceSeconds:body.graceSeconds,...(body.apiKey===undefined?{}:{apiKey:body.apiKey})};return db.updateWorkspaceResendSettings(current.workspaceId,input,key)});
  ...
  api.get("/api/v1/endpoints/:id/napcat/status", async request => { const current = await authorize(request, "operate"); const snapshot = await db.endpointNapcatSnapshot(current.workspaceId, idParams.parse(request.params).id),checkedAt=new Date().toISOString(),staleAfterSeconds=15,observationFresh=snapshot.observationAt!==null&&Date.parse(checkedAt)-Date.parse(snapshot.observationAt)<=staleAfterSeconds*1000;return { qq: snapshot.metadata.qq ?? null, login: snapshot.metadata.login ?? null, onebot: snapshot.metadata.onebot ?? null, traffic: snapshot.metadata.traffic ?? null,freshness:{observationAt:snapshot.observationAt,nodeHeartbeatAt:snapshot.nodeHeartbeatAt,checkedAt,staleAfterSeconds,fresh:snapshot.nodeOnline&&observationFresh} }; });
