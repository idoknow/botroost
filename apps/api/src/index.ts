...
export function buildApi(options: ApiOptions = {}): FastifyInstance {
  ...
  api.get("/api/v1/endpoints/:id/napcat/status", async request => { const current = await authorize(request, "operate"); const metadata = await db.endpointNapcatMetadata(current.workspaceId, idParams.parse(request.params).id); return { qq: metadata.qq ?? null, login: metadata.login ?? null, onebot: metadata.onebot ?? null, traffic: metadata.traffic ?? null }; });
