...
describe("notification durability schema",()=>{
  it("adds state and retryable notification outbox in a new migration",async()=>{
    ...
    for(const fragment of ["napcat_notification_state","notification_outbox","provider_message_id","attempts integer","failed_at"])expect(migrationSql).toContain(fragment);
