...
import { digest, type PostgresDatabase, type Role } from "@botroost/database";
...
export class AuthService {
 ...
 async addMember(workspaceId:string,email:string,password:string,role:Role){if(password.length<12)throw new Error("password too short");return this.db.addMember(workspaceId,{email,passwordHash:await argon2.hash(password,{type:argon2.argon2id}),role})}
