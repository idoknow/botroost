...
if [ "$#" -gt 0 ]; then exec setpriv --reuid=node --regid=node --init-groups "$@"; fi
case "${BOTROOST_PROCESS:-}" in
  ...
  bootstrap)
    ...
    exec setpriv --reuid=node --regid=node --init-groups node apps/api/dist/cli.js bootstrap --email "$BOOTSTRAP_EMAIL" --workspace "$BOOTSTRAP_WORKSPACE" <<EOF
${BOOTSTRAP_PASSWORD:-}
EOF
    ;;
